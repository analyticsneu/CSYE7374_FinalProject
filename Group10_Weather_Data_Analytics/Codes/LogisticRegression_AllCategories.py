...
## Function to remove N/A's and empty values
#
def remove_na(line):
...
def mappingfunction(line):
    ...
    new_direction = str(values[4])
    
    values[1] = time_list[values[1]]
    new_time = str(values[1])
    
    updated_list = [values[0]] + [new_time] + values[2:4] + [new_direction] + values[5:10]
...
## Creating classification model & calculating Test Error and weights
#
for output in weather_output: 
    lp_data = updated_data.map(lambda line: parse_Data(line,output))
    training, test = lp_data.randomSplit([0.7,0.3])
    model = LogisticRegressionWithLBFGS.train(training)
         
    list_weights.append(model.weights)
    
    labelsAndPreds = test.map(lambda p: (p.label, float(model.predict(p.features))))
    testError = labelsAndPreds.filter(lambda (v, p): v != p).count() / float(test.count())
    print ("Test Error: "+ str(testError))
    
    
## Predicting class using weight matrix 
#
...
for weight in list_weights:
    weighted_values = np.dot(np.transpose(test_list),weight)
    print ("Weighted Value: "+ str(weighted_values))